...
injectScript(function() {
  ...
  (function() {
    ...
    return tid = setInterval((function() {
      var _ref;
      if (((_ref = window.tinymce) != null ? _ref.get('editor') : void 0) != null) {
        clearInterval(tid);
        return boot();
      }
    }), 1000);
  ...
  return boot = function() {
    var rrmd;
    return window.rrmd = rrmd = new function() {
      var titleBgEl, titleEl,
        _this = this;
      this.editor = window.tinymce.get('editor');
      this.conv = function(md) {
        return marked(md);
      };
      this.transform = function(h) {
        var codeEl, content, el, fontSizeList, hEl, i, preEl, presEl, _i, _j, _k, _len, _len1, _ref, _ref1, _ref2;
        el = document.createElement('div');
        el.innerHTML = h;
        _ref = [].slice.call(el.getElementsByTagName('pre'));
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          preEl = _ref[_i];
          if (((_ref1 = (codeEl = preEl != null ? preEl.firstElementChild : void 0)) != null ? _ref1.tagName : void 0) === 'CODE') {
        ...
        for (i = _j = 1; _j <= 6; i = ++_j) {
          _ref2 = [].slice.call(el.getElementsByTagName('h' + i));
      ...
      this.embed = function(h, md) {
        return h + ("<span style=\"visibility: hidden; display: block; height: 0; background-image: url('http://dummy/$rrmd$')\">" + (str_to_b64(md)) + "</span>");
      };
      this.unembed = function(h) {
        var b64, el, list, spanEl;
        el = document.createElement('div');
        el.innerHTML = h;
        list = (function() {
          var _i, _len, _ref, _results;
          _ref = el.getElementsByTagName('span');
          _results = [];
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            spanEl = _ref[_i];
            if (/\$rrmd\$/.test(spanEl.style.backgroundImage)) {
              _results.push(spanEl);
            }
          }
          return _results;
        })();
        b64 = list.length >= 1 ? list[0].innerHTML : '';
        try {
      ...
      this.uiHtml = "<div id=\"rrmd_wrapper\" style=\"margin: 0em 0em 1em 0em\">\n  <textarea id=\"rrmd_area\" style=\"font-family: Consolas, 'Courier New';\" placeholder=\"Type markdown _here_!\"></textarea>\n</div>";
      this.uiSpanEl = document.createElement('span');
      this.uiSpanEl.innerHTML = this.uiHtml;
      this.uiBefore = document.getElementById('editor_tbl');
      this.uiBefore.parentElement.insertBefore(this.uiSpanEl, this.uiBefore);
      this.uiAreaEl = document.getElementById('rrmd_area');
      this.uiAreaEl.innerHTML = this.unembed(this.editor.getContent());
      this.uiAreaEl.oninput = function() {
        if (_this.tid != null) {
          clearTimeout(_this.tid);
          _this.tid = null;
        }
        return _this.tid = setTimeout(_this.fill(), Math.min(_this.uiAreaEl.value.length / 50, 500));
      };
      ...
      this.fill = function() {
        var htmlEmbed, htmlInit, htmlTrans, md;
        try {
          md = this.uiAreaEl.value;
          htmlInit = this.conv(md);
          htmlTrans = this.transform(htmlInit);
          htmlEmbed = this.embed(htmlTrans, md);
          return this.editor.setContent(htmlEmbed);
        } catch (e) {
